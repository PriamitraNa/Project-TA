...
/**
 * Check if guru is wali kelas of specific kelas
 */
...
/**
 * Check if guru is wali kelas of specific siswa
 */
...
/**
 * Get nilai akademik all mapel for siswa
 */
...
/**
 * Get rekapitulasi absensi for siswa
 * Note: absensi table doesn't have tahun_ajaran_id/semester, 
 * we filter by kelas_id from current active semester
 */
...
/**
 * Get catatan perkembangan for siswa
 * Note: Using catatan_header table, filter by kelas_id from current active semester
 * Get first message from each catatan thread
 */
...
/**
 * Check if guru mengampu mapel di kelas tertentu
 */
...
export const getRekapNilaiSiswa = (kelasId, mapelId, tahunAjaranId, semester) => {
  return new Promise((resolve, reject) => {
    ...
    console.log('🔍 getRekapNilaiSiswa called with:', {
      kelasId,
      mapelId,
      tahunAjaranId,
      semester
    ...
    db.query(
      ...
      (error, results) => {
        ...
        // Debug: Log results for first student
        if (results.length > 0) {
          console.log('📊 First student data:', {
            siswa_id: results[0].siswa_id,
            nama: results[0].nama,
            nilai_id: results[0].nilai_id,
            nilai_kelas_id: results[0].nilai_kelas_id,
            nilai_mapel_id: results[0].nilai_mapel_id,
            nilai_ta_id: results[0].nilai_ta_id,
            nilai_semester: results[0].nilai_semester,
            lm1_tp1: results[0].lm1_tp1,
            lm1_tp2: results[0].lm1_tp2
          });
        }
