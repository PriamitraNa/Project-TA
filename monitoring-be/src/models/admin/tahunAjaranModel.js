...
export const getAllTahunAjaran = (sortBy = 'tahun', order = 'DESC') => {
  return new Promise((resolve, reject) => {
    ...
    // Jika sortBy adalah tahun, tambahkan sorting semester sebagai secondary
    if (sortBy === 'tahun') {
      query += `, semester ASC`;
    }
...
export const getTahunAjaranAktif = () => {
  return new Promise((resolve, reject) => {
    ...
    db.query(query, (err, results) => {
      if (err) return reject(err);
      resolve(results[0]); // Return single object, not array
    });
  });
};
...
export const toggleTahunAjaranStatus = (id) => {
  return new Promise((resolve, reject) => {
    // Ambil data tahun ajaran terlebih dahulu
