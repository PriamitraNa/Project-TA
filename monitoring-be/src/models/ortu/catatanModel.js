...
/**
 * Get statistik catatan by siswa_id
 * Count total, positif, negatif, netral (all-time)
 */
...
/**
 * Get list catatan by siswa_id with pagination, filter, and search
 */
export const getCatatanList = (siswaId, filters) => {
  return new Promise((resolve, reject) => {
    ...
    // Search by guru_nama or isi catatan (first message)
    if (search) {
 ...
 * Include header info (guru, kelas, mapel, kategori, jenis, status)
 */
 ...
 * Returns thread komunikasi antara guru dan ortu
 */
 ...
 * Called when ortu opens catatan detail (auto mark as read)
 */
...
/**
 * Verify catatan exists and get basic info
 * Used for authorization check before adding reply
 */
