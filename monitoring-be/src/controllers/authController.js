...
/**
 * Change Default Password Controller
 * Endpoint untuk user yang harus ganti password (first login atau after reset)
 * Menggunakan temp_token dari login response
 */
export const changeDefaultPassword = async (req, res, next) => {
  const { new_password, confirm_password } = req.body
  const userId = req.user.id // Dari temp token yang sudah di-decode di middleware
