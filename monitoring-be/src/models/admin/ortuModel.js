...
// Get all orangtua data with pagination, search, filter, sorting, and statistics
export const getAllDataOrtu = (
  ...
  return new Promise((resolve, reject) => {
    ...
    // Handle sorting - Primary sort by NIK, Secondary sort by Nama
    ...
    // Add secondary sort by nama_lengkap to ensure consistent pagination
    ...
    db.query(countQuery, countParams, (err, countResult) => {
      ...
      db.query(query, params, (err, results) => {
        ...
        // Get anak data for each orangtua (one-to-many relationship)
        if (results.length > 0) {
          ...
          db.query(anakQuery, ortuIds, (err, anakResults) => {
            ...
            // Add anak data and jumlah_anak to each orangtua
...
export const getOrtuById = (id) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(query, [id], (err, results) => {
      ...
      // Get anak data (one-to-many relationship)
      const anakQuery = `
        SELECT 
          s.id,
          s.nama_lengkap,
...
// Get orangtua by ID with complete children information (for edit modal)
export const getOrtuByIdWithAnak = (id) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(query, [id], (err, results) => {
      ...
      // Get anak data with complete information (kelas and tahun ajaran)
      const anakQuery = `
        SELECT 
          s.id,
          s.nama_lengkap,
...
// Get available students for edit modal (include students already related to current parent)
export const getAvailableStudentsForEdit = (search = "", limit = 50, excludeIds = [], includeIds = []) => {
  return new Promise((resolve, reject) => {
    ...
    // Handle exclude IDs (students already selected in current form)
    if (excludeIds && excludeIds.length > 0) {
      const placeholders = excludeIds.map(() => '?').join(',');
      whereClauses.push(`s.id NOT IN (${placeholders})`);
      params.push(...excludeIds);
...
// Check single NIK orangtua with exclude (for edit modal)
      ...
      const insertPromises = validData.map((ortu, index) => {
        return new Promise((resolveInsert, rejectInsert) => {
          db.query(insertQuery, [ortu.nama_lengkap, ortu.nik, ortu.kontak, ortu.relasi], (err, result) => {
            ...
            // Insert anak relationships if any
...
// Update orangtua with anak relationships
  ...
  return new Promise((resolve, reject) => {
    db.beginTransaction((err) => {
      ...
      db.query(updateOrtuQuery, ortuParams, (err, result) => {
        ...
        // Delete existing anak relationships
        ...
        db.query(deleteAnakQuery, [ortuId], (err) => {
          ...
          // Insert new anak relationships if any
          if (anakData && anakData.length > 0) {
            ...
            // No anak data, just commit
            db.commit((err) => {
              if (err) {
                return db.rollback(() => reject(err));
              }

              // Get updated data without anak
              getOrtuByIdWithAnak(ortuId)
                .then(updatedOrtu => resolve(updatedOrtu))
                .catch(reject);
            });
...
// Delete orangtua with anak relationships
export const deleteOrtuWithAnak = (ortuId) => {
  return new Promise((resolve, reject) => {
    db.beginTransaction((err) => {
      if (err) return reject(err);

      // First, get ortu data before deletion for response
      ...
      db.query(getOrtuQuery, [ortuId], (err, ortuResults) => {
        ...
        db.query(getAnakQuery, [ortuId], (err, anakResults) => {
          ...
          // Delete anak relationships first
          const deleteAnakQuery = `DELETE FROM orangtua_siswa WHERE orangtua_id = ?`;
