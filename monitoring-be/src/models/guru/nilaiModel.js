 ...
 * Filter by kelas_id dan tahun ajaran aktif
 */
...
/**
 * Get daftar siswa beserta nilai-nilai mereka untuk kelas dan mapel tertentu
 */
...
/**
 * Save single nilai cell (UPSERT)
 * @param {Object} data - { siswa_id, kelas_id, mapel_id, tahun_ajaran_id, semester, field, nilai, updated_by }
 * @returns {Promise}
 */
export const simpanCell = (data) => {
  return new Promise((resolve, reject) => {
    ...
    // Strategy: 
    // 1. First, ensure row exists (INSERT with all NULL if not exists)
    // 2. Then, UPDATE the specific field
    ...
    db.query(
      ...
      (insertError) => {
        ...
        // Step 2: Update the specific field
        const updateQuery = `
          UPDATE nilai
          SET ${field} = ?,
              updated_by = ?,
