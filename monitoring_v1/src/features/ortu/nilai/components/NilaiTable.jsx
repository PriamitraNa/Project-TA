      ...
      >
        ...
        <thead>
          ...
          <tr className="bg-gradient-to-r from-slate-600 to-slate-700 text-white">
            <th
              rowSpan="3"
              className="sticky left-0 z-20 bg-gradient-to-r from-slate-50 to-slate-100 px-2 md:px-3 py-3 text-center text-xs font-bold text-gray-700 uppercase tracking-wider border-r-2 border-gray-300"
            ...
            <th
              rowSpan="3"
              className="sticky z-20 bg-gradient-to-r from-slate-50 to-slate-100 px-2 md:px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider border-r-2 border-gray-300"
              style={{ left: '40px', minWidth: '150px', width: '150px' }}
            >
              Mata Pelajaran
            </th>
            ...
            return (
              <tr key={row.id} className="group hover:bg-gray-50 border-b border-gray-100">
                ...
                <td 
                  ...
                  style={{ left: '40px', minWidth: '150px', width: '150px' }}
                >
                  <div className="truncate" title={row.nama_mapel}>
                    {row.nama_mapel}
                  </div>
                </td>
                    ...
                    <div className="flex">
                      {['tp1', 'tp2', 'tp3', 'tp4'].map((tp) => {
                        ...
                        return (
                          <div 
                            key={`${lm}_${tp}`} 
                            className={`w-[55px] px-2 py-2 text-center text-xs md:text-sm ${getColorClass(value)}`}
                          >
                            {formatNumber(value)}
                          </div>
                        );
                ...
                {['lm1', 'lm2', 'lm3', 'lm4', 'lm5'].map((lm, index) => {
                  ...
                  return (
                    <td 
                      key={`${lm}-ulangan`} 
                      className={`px-2 py-2 text-center text-xs md:text-sm ${getColorClass(value)} ${index === 4 ? 'border-r-2' : 'border-r'} border-gray-200`}
                      style={{ minWidth: '70px' }}
                    >
                      {formatNumber(value)}
                    </td>
                ...
                {/* UTS (READ-ONLY) */}
                <td 
                  className={`px-2 py-2 text-center text-xs md:text-sm ${getColorClass(row.uts)} border-r border-gray-200`}
                  style={{ minWidth: '70px' }}
                >
                ...
                {/* UAS (READ-ONLY) */}
                <td 
                  className={`px-2 py-2 text-center text-xs md:text-sm ${getColorClass(row.uas)} border-r border-gray-200`}
                  style={{ minWidth: '70px' }}
                >
                ...
                <td className="px-2 py-3 text-center" style={{ minWidth: '70px' }}>
                  <span className={`inline-flex items-center justify-center px-2 py-1 rounded-full text-xs font-bold border ${getGradeColor(grade)}`}>
                    {grade}
                  </span>
                </td>
              </tr>
