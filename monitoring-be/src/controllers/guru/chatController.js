...
export const sendMessage = async (req, res, next) => {
  try {
    const guruId = await getGuruId(req);
    const userId = req.user.id; // user_id untuk sender_id
    const { id } = req.params;
    ...
    const result = await chatService.sendMessageService(guruId, userId, parseInt(id), { message });
...
export const createConversation = async (req, res, next) => {
  try {
    ...
    // Status code: 201 if new, 200 if existing
